...
function getAccessToken(): string {
  const token = "eyJhbGciOiJIUzI1NiJ9.eyJhdWQiOiJkNjAyNmUzOTNlYjUyNDNhZjNjZjg0MjExYWNkNDZmZSIsIm5iZiI6MTY3NTA0MTc0MS45ODUsInN1YiI6IjYzZDcxYmNkMTdjNDQzMDA3YjY4MzQ3YSIsInNjb3BlcyI6WyJhcGlfcmVhZCJdLCJ2ZXJzaW9uIjoxfQ.6qAP27OpriPcApMqskIRrxyjHk0NFa0gyTm2Cc3BESE";
...
function normalizeMovie(item: TMDBMovieResponse): Media {
  ...
  return {
    id: item.id,
    mediaType: 'movie',
    title: item.title?.trim() || item.original_title?.trim() || `Movie ${item.id}`,
    originalTitle:
  ...
  try {
    ...
    const message =
      error instanceof Error
        ? error.message
        : 'Unknown TMDB request error';

    throw new TMDBError(
      `Failed to fetch from TMDB: ${message}`,
      null,
      endpoint,
    );
  }
...
export async function getPopularMovies(
  ...
  const response = await fetchFromTMDB<
    TMDBRawListResponse<TMDBMovieResponse>
  >('/movie/popular', {
...
export async function getPopularTVShows(
  ...
  const response = await fetchFromTMDB<
    TMDBRawListResponse<TMDBTVResponse>
  >('/tv/popular', {
    page: validatePage(page),
    language,
  });
...
export async function searchMedia(
  ...
  const response = await fetchFromTMDB<
    TMDBRawListResponse<TMDBSearchResult>
  >('/search/multi', {
